...
%macro McNemar (tabIn=,ID=subject,comparaison=&col,value=&value.,byvar=&row2,start=1,end=)/store source; 
...
%if %length(&end.) ne 0 %then %do; %let NB&comparaison.=&end.; %end;
			...
			run;
				...
				%do w=&start. %to %eval (&&NB&comparaison. -1) ;
