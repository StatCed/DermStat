...
OBJET : génère les statistiques descriptives des données quantitatives
		fournit égelement, les test de comparisons Wilcoxon/Mann-Witney,
		et le test de student

		c'est une image de la macro %stat() utilisée pour l'analyse des
		données provenant de EVIC 

REQUIERE : 	prend en entrée une table SAS de données avec les variables numériques
