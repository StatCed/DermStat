...
%macro creation_formats(tab=, Modified_format=, exclu=)
 / STORE SOURCE;

ods output position=position;
proc contents data=&tab order=varnum;
run;
...
%if %symexist(&exclu) ne 0 %then %do;
 	if variable not in (&exclu);
%end;
...
%macro analyse_descriptive(tab=DATA_3, class_data=class) / STORE SOURCE;
...
data Appel_Macro_DESC;
	...
	if (type = 'Char' and index(var_in_classdata,trim(variable))=1) then do;
		Appel_Macro_DESC=catx(' ','%freq(table=&tab, para=',VARIABLE,', num=',obs, ', f=',cats(VARIABLE,'., class=&class_data);')); 
	end;

	else if (type = 'Char' and index(var_in_classdata,trim(variable))=0) then do;
